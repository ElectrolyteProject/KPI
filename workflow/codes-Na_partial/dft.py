...
def analyze_log_na(log_path: str) -> Dict[str, Any]:
    '''Analyze a Gaussian log file of Na+ and extract required properties.
    ...
    if log_path.endswith('_sol.log'):
        extracted = {
            'HFenergy_Na+_sol': str(HFenergy),
            'thermochemistry_Na+_sol': thermoenergy,
        }
    else:
        extracted = {
            'HFenergy_Na+_RESP2': str(HFenergy),
            'thermochemistry_Na+_RESP2': thermoenergy,
        }
    return extracted
...
def _append_na_sites(mol):
    """
    Add a Na atom to pristine molecules
    Currently, only N, O and F sites are considered as the binding sites #20230817 N added
    Return: The lists of new molecules with a Na
    """
    Na_O = _o_sites(mol, d=2.0)
    Na_F = _f_sites(mol, d=2.0)
    Na_N = _n_sites(mol, d=2.0)
    Na_multi = _multi_interaction_sites(mol)
    Na_sites = Na_O + Na_F + Na_N + Na_multi

    new_mols = []
    structs = []
    # The molecule is first converted into structure and dedup
    # and then converted back to molecule
    for i, site_i in enumerate(Na_sites):
        new_mol = deepcopy(mol)
        new_mol.append(species='Na', coords=site_i)
