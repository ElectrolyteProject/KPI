...
import argparse
import time
import dft, md, assist, local_fs, bzip2_files
import traceback
import logging
import glob
logging.basicConfig(
...
def workflow(mol_name, scaling_factor=0.7):
    zero_time = time.time()
    ...
    try:
        ...
        logging.info(f'--- Task6 on {mol_name}: DFT optimization of Na+ in solvent environment (RESP2) ---')
        ...
        mol_info = {'name': f'{mol_name}-Na+', 'coordinate': ["Na   0.0   0.0   0.0"]}
        ...
        if args.start <= 6 and len(molinfo) == 0:
            ...
            files = dft.job(dir_moldft_Na, mol_info, '1 1', keywords, solva_info=metadata,
                            sc=config['supercomputer'], ncores=ncores_dft)
            ...
            # Update time
            totaltime = dft._extract_elapsedtime(files[0])
            if metadata.get('time(hour)'):
                metadata['time(hour)'] += totaltime
                round(metadata['time(hour)'], 4)
            else:
                metadata.update({'time(hour)': totaltime})

            # Move generated files to the output folder
            local_fs.write_json_file(os.path.join(dir_moldft_Na, mol_name + '.molinfo'), metadata)
            _ = bzip2_files.bzip2_parallel([files[0]])
            logging.info(f'--- Task6 on {mol_name}: {time.time() - start_time:.4f} s ---')
        ...
        try:
            ...
            if sampid == 0:
                ...
                metadata = local_fs.load_json_file(
                    os.path.join(dir_moldft, f'moldft3_{sampid - 1}', mol_name + '.molinfo'))
            if metadata.get('coordinate_samp'):
                start_time = time.time()
                logging.info(f'--- Task7_{sampid} on {mol_name}: '
                             f'DFT optimization of multiple ion-solvent clusters (RESP2) ---')
                # metadata = local_fs.load_json_file(mol_name)
                mol_info = {'name': f'{mol_name}+Na+', 'coordinate': metadata['coordinate_samp'][0]}
                ...
                if args.start <= 7 and len(molinfo) == 0:
                    ...
                    try:
                        keywords = dft.keywords(2)
                        files = dft.job(dir_moldft3, mol_info, '1 1', keywords, solva_info=metadata,
                                        sc=config['supercomputer'], ncores=ncores_dft)
                    ...
                    local_fs.write_json_file(os.path.join(dir_moldft3, mol_name + '.molinfo'), metadata)
                    _ = bzip2_files.bzip2_parallel([files[0]])
                    logging.info(f'--- Task7_{sampid} on {mol_name}: {time.time() - start_time:.4f} s ---')

                df_js.at[len(df_js) - 1, f'moldft3_{sampid}'] = 0
                df_js.to_csv(dir_js, index=False, header=True)
                success += 1
            else:
            ...
            local_fs.write_json_file(os.path.join(dir_moldft3, mol_name + '.molinfo'), metadata)
            df_js.at[len(df_js) - 1, f'moldft3_{sampid}'] = 1
            df_js.to_csv(dir_js, index=False, header=True)
            continue
    moldft3_fchks = glob.glob(os.path.join(dir_moldft, 'moldft3_*', '*.fchk*'))
    ...
    try:
        start_time = time.time()
        logging.info(f'--- Task8 on {mol_name}: MD simulation of electrolyte ---')
        ...
        if args.start <= 8 and len(molinfo) == 0:
            ...
            while True:
                try:
                    model_path = md.mkfile_modelxyz(dir_molmd2, model, [xyz_path, sxyz_path], mol_nums,
                                                    round(box_size * pow(scaling_factor, 1 / 3), 2))
                    break
                except:
                    scaling_factor += 0.05
            ...
            try:
                ...
                args.start = 8
                scaling_factor -= 0.05
                return workflow(mol_name, scaling_factor=scaling_factor)
            ...
            _ = bzip2_files.bzip2_parallel(files)
            logging.info(f'--- Task8 on {mol_name}: {time.time() - start_time:.4f} s ---')
    ...
    try:
        ...
        mol_info = {'name': f'{mol_name}+Na+', 'coordinate': metadata['coordinate_cls'][min_index]}
    ...
    logging.info(f'=== {mol_name}: {time.time() - zero_time:.4f} s ===\n')
...
if __name__ == "__main__":
    ...
    for mol in allmols:
        try:
            ...
            continue
    logging.info(f'==================== END {time.time() - zero_time_tot:.4f} s ====================')
